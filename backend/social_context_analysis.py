...
CITY_ALIASES = {
    ...
    "castelló de la plana": "castellon",
}
...
def _normalise_city(city: str) -> str:
    key = " ".join(city.lower().strip().split())
    try:
        return CITY_ALIASES[key]
    except KeyError as error:
        raise ValueError(
            "city must be 'valencia' or 'castellon'"
        ) from error
...
@lru_cache(maxsize=2)
def load_opinions(city: str) -> list[dict[str, Any]]:
    city_id = _normalise_city(city)
    records = _read_json(DATA_ROOT / city_id / "public_opinions.json")
...
@lru_cache(maxsize=2)
def load_overlays(city: str) -> dict[str, Any]:
    city_id = _normalise_city(city)
    geojson = _read_json(DATA_ROOT / city_id / "unsafe_area_overlays.geojson")
...
def analyse_social_context(
    *,
    ...
    return {
        ...
        "method_note": (
            "Every geographically matching, rating-eligible opinion contributes. "
